    ...
    class HiloViewModel: ObservableObject {
        ...
        private func calculateChances() {
            ...
            for card in deck {
                if card.rank > current.rank { higher += 1 }
                if card.rank < current.rank { lower += 1 }
            }

            let total = higher + lower

            higherChance = total > 0 ? (Double(higher) / Double(total)) * 100 : 0
            lowerChance = total > 0 ? (Double(lower) / Double(total)) * 100 : 0
        }
